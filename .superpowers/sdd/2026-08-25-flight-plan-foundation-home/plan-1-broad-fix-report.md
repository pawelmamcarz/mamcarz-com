...
Both Important Plan 1 review findings are closed locally after two fix rounds. Public claim enforcement is registry-driven and fail-closed for every declared surface, high-risk approved claims are bounded to controlled exact units instead of substring matches, direct static JavaScript literals are decoded before matching, and the back-to-top click path follows the runtime reduced-motion preference.
